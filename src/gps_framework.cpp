...
void GpsFramework::processPilot(double deplacementX, double deplacementY
    ...
    if(m_etat == Etat_OK){
        if(m_line){
            double dist = m_lineAB.distance(m_tracteur.m_pt_antenne_corrige->m_x, m_tracteur.m_pt_antenne_corrige->m_y,m_deplacementX, m_deplacementY);
            setDistance(dist);
            ...
            if(m_pilot_algo == AlgoPilot::FollowCarrot){
                ...
                m_angle_correction = m_curveAB.calculRearWheelPosition(essieu_arriere_x, essieu_arriere_y, essieu_avant_x, essieu_avant_y, m_deplacementX, m_deplacementY, m_vitesse, 1.5, m_pilot_rwp_kth, m_pilot_rwp_kte);
            } else {
                m_angle_correction = 0;
            }
        }
